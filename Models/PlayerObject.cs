...
namespace TheAdventure.Models
{
    public class PlayerObject : RenderableGameObject
    {
        ...
        public PlayerObject(SpriteSheet spriteSheet, int x, int y, Engine engine) : base(spriteSheet, (x, y))
        {
            _engine = engine;
        ...
        public void LaunchProjectile(int textureId, int width, int height)
        {
            var direction = State.Direction switch
            {
                PlayerStateDirection.Up => (0, -1),
                PlayerStateDirection.Down => (0, 1),
                PlayerStateDirection.Left => (-1, 0),
                PlayerStateDirection.Right => (1, 0),
                _ => (0, 0)
            };

            var projectile = new ProjectileObject(textureId, width, height, 2.0, Position, direction, 300.0);
            _engine.AddProjectile(projectile);
        }
    }
}
